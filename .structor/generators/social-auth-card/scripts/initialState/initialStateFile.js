    ...
    traverseWithResult(ast, (node, res) => {
        if( node.type === 'Property' && node.key && node.key.type === 'Identifier' && node.value){
            if(node.value.type === 'ObjectExpression' || node.value.type === 'ArrayExpression'){
                let key = res.____key + '.' + node.key.name;
                res[node.key.name] = {
                    ____key: key
                };
                result.set(key, { parentKey: res.____key, node });
...
export function mergeInitialStateWithPart(initialStateAstNode, partStateAst){

    let partStateMap = getObjectAstMap(partStateAst);
    let initialStateMap = getObjectAstMap(initialStateAstNode);
    mergeMetaAstToInitialAst(partStateMap, initialStateMap, initialStateAstNode);
