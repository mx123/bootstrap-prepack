...
export function injectNonExistingReducer(ast, reducerRoot, componentName, actionsFilePath) {
    ...
    if (defaultNodeAst.callee
        ...
        if (defaultNodeAst.arguments[0].properties.length > 0) {
            defaultNodeAst.arguments[0].properties.forEach(prop => {
                if (prop.key && prop.key.name === reducerRoot) {
                    //console.log('I found reducer: ' + reducerRoot);
                    if(prop.value){
